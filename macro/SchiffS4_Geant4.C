...
void SchiffS4_Geant4(){

  gROOT->Reset();

#define MAX_nentries 500;
...
//
 ...
 for (Int_t i=0; i<MAX_nentries; i++){
  ...
  TH1F* hS2  = new TH1F("hS2", "hS2",MAX_nentries,4.0,9.0);
  G->Draw("gEnergy>>hS2","gEnergy>4.0","");
  hS2->Scale(scale); // 
  ...
  hS2->SetMinimum(1.0e+3);
  hS2->SetMaximum(1.0e+9);
  
  hS2->GetXaxis()->CenterTitle();
  hS2->GetXaxis()->SetTitleOffset(1.2);
  hS2->GetXaxis()->SetTitleSize(0.06);
  TString tit = "Photon Energy (MeV)";
  hS2->GetXaxis()->SetTitle(tit.Data());

  hS2->GetYaxis()->CenterTitle();
  hS2->GetYaxis()->SetTitleOffset(1.0);
  hS2->GetYaxis()->SetTitleSize(0.06);
  TString tit ="n_{#gamma} /(#muA 0.01 MeV)";
  hS2->GetYaxis()->SetTitle(tit.Data());
 ...
 grSchiff->SetLineStyle(2);
 grSchiff->SetMarkerStyle(1);
 
 grSchiff->Draw("L same");

 ////

 leg = new TLegend(0.40,0.60,0.6,0.7);
 leg->SetFillColor(0);
 leg->SetShadowColor(0);
 leg->AddEntry(hS2,"GEANT4","L");
 leg->AddEntry(grSchiff,"Schiff","L");
 leg->Draw();
 leg->SetHeader("T_{e} = 8.5 MeV, 2.0 mm");
 leg->Draw();
